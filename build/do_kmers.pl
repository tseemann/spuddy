...
# -3 = lines common to FILE 1 and 2
my $COMM = "LC_ALL=C comm --nocheck-order";
my $SED = "LC_ALL=C sed -E";
...
say "export CSVTK_THREADS=1";
say "export SEQKIT_THREADS=1";
say "export LC_ALL=C"; 
say 'CPUS=$(( $(nproc) / 2 ))'; # hope we have > 1
say "RAMGB=\$(( \$(free -wg | awk '/Mem/ { print \$2 }') / 2 ))";
say "TEMPDIR=/dev/shm/spuddy.\$\$";
say "mkdir -p \$TEMPDIR";
...
banner("Finding distinct kmers per isolate");
say "$PARAJ -a $gcmd";
...
say "$PARAJ -a $G ".dq(clean("
    [[ $CHECK '{}/$name' ]] ||
      $COMM -12 $name.kmers {}/$name
    > {}/$name.uniq
    "));
# comm is 20x faster than the sort/uniq
# say "$PARAJ -a $G ".dq(clean("
#      $KMSORT $name.kmers {}/$name
#    | $UNIQ -d 
#    > {}/$name.uniq
#    "));
...
say "$PARAJ -a $G 'wc -l {}/$name.uniq' | sort -bnr > $name.anno";
